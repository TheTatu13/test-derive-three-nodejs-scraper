...
Fill this in per derived company. Check `https://example.com/robots.txt` before the
...
| Path | Role |
|---|---|
| `https://example.com/careers` | Public list of open positions (server-rendered HTML) |
| `` | Job sitemap — canonical `https://example.com/jobs/<slug>/` permalinks |
| `https://example.com/jobs/<slug>/` | Individual job pages (only referenced, not fully crawled) |
